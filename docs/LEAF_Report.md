...
我们在 ReAct 架构的检索增强翻译系统中发现了一种稳定的失败模式——**查询漂移（Query Drift）**：LLM 在迭代推理过程中擅自改写检索查询，导致精确匹配失败，漂移率高达 35%，其中 71% 的漂移直接导致翻译错误。LEAF 通过四层品质保证架构（CoreVocab / RAG / Grammar / Verify）和 Multi-Agent 编排机制，将查询漂移率从 35% 降至 0%，翻译准确率从 65% 提升至 **95%**。消融实验证明，30 个百分点的提升全部来自架构设计本身（Orchestrator 控制查询透传），而非单一组件或 prompt engineering。
...
LEAF 采用 Multi-Agent 架构，包含 4 个专职 Agent：

- **Orchestrator Agent**：系统总管，接收用户输入，通过 ReAct 循环决定调用哪个 Agent，确保原始查询不被改写
- **Knowledge Agent**：知识检索与翻译，集成 RAG + CoreVocab + ExactMatch
- **Quality Agent**：品质控制，执行反向翻译验证（Reverse Verification）
- **Teaching Agent**：教学与评估
...
1. **查询透传**：Orchestrator 的 system prompt 明确指示"用户原始输入直接作为 translate 的 text 参数"，且在代码层面确保查询原封不动传递
2. **结构化工具调用**：Orchestrator 的工具 schema 设计使得 LLM 无需（也无法）改写查询参数
...
**关键发现**：配置 #3→#4 是唯一出现准确率**下降**的步骤——引入 ReAct 后反而降了 15pp。这是因为 LLM 的"自由推理"能力改写了检索查询。而配置 #4→#6 的 +25pp 则是 Orchestrator 架构约束带来的恢复。
...
| **Architecture（整个 Orchestrator→SA+ReAct）** | **65%** | **-30pp** | **架构本身是核心贡献者** |

**95% 的提升归因**：架构控制 +25pp（Orchestrator 确保查询不漂移），系统 prompt +5pp，其余组件 0pp。
...
LEAF 的 Orchestrator 通过以下架构设计解决了这个问题：

- **职责分离**：Orchestrator 负责"决定做什么"，Knowledge Agent 负责"怎么做"，查询内容不在 Orchestrator 的修改范围内
...
│   ├── agents/            # Orchestrator, Knowledge, Quality, Teaching
